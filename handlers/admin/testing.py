...
# --- GET PLACEHOLDER ---
@admin_router.message(Command("get_placeholder"))
async def cmd_get_placeholder(message: types.Message):
    if not is_admin(message.from_user.id): return
    
    filename = "temp_video_ph.mp4"
    wait_msg = await message.answer("📤 Генерирую видео...")
    try:
...
@admin_router.message(Command("get_audio_placeholder"))
async def cmd_get_audio_placeholder(message: types.Message):
    ...
    try:
        ...
        msg = await message.answer_audio(FSInputFile(file_path), title="Searching...", performer=f"@{bot_info.username}")
